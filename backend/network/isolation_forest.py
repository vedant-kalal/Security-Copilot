...
persists the fitted estimator + scaler to
...
NOT YET REWORKED for the security-copilot spec (kept as a ported
reference — see project memory: security-copilot-reuse-from-sentinelai).
Two things still need to change before this matches spec section 5.2
exactly: (1) hyperparameters should be
`IsolationForest(n_estimators=200, max_samples=256, contamination=0.02,
random_state=42)` — currently missing `max_samples` and using
`contamination=0.05`; (2) the escalation threshold should be the 99th
percentile of `score_samples()` on held-out normal traffic, recomputed
on every retrain — currently a fixed sigmoid-normalized cutoff. Also,
`feature_engineering.py`'s 10 features are CICIDS2017-column-oriented;
spec section 5.1 wants psutil-connection-derived features instead
(connection count, unique dest/port count, failed-ratio, sin/cos hour,
one-hot protocol) — that's a bigger rework, tied to `flow_collector.py`
being built first.
"""
...
from network.feature_engineering import FEATURE_NAMES, extract_features
